...
export default function NotificationCard({
  ...
  // Check if this is a failing student notification
  const isFailingStudent = type === 'failing_student';
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pr-24 sm:pr-28">
          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 flex-1 min-w-0">
            <span className={`font-bold text-sm sm:text-base break-words ${
              isFailingStudent ? 'text-red-600' : 'text-inherit'
            }`}>
              {title}
            </span>
            {!readStatus && (
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <p className={`text-xs sm:text-sm whitespace-pre-wrap break-words ${
                isFailingStudent ? 'text-red-600 font-medium' : 'text-gray-700'
              }`}>
                {description}
              </p>
            </div>
