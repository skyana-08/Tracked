...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        echo json_encode(["success" => true, "message" => "Professor information updated successfully"]);
    } else {
        echo json_encode(["success" => false, "message" => "No changes made or professor not found"]);
    }
} else {
    echo json_encode(["success" => false, "message" => "Error updating professor: " . $conn->error]);
}
