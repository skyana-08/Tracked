...
try {
    ...
    $sql = "UPDATE classes SET status = 'Archived' WHERE subject_code = ? AND professor_ID = ?";
    $stmt = $pdo->prepare($sql);
    $result = $stmt->execute([$subject_code, $professor_ID]);

    if ($result && $stmt->rowCount() > 0) {
        $response['success'] = true;
        $response['message'] = 'Class archived successfully';
    } else {
        throw new Exception('Class not found or already archived');
    }
